...
Session Rule creation raises the consequence of trusting a forged request: a human who remembers it can create a daemon-wide rule that affects later genuine requests. The webhook cannot create, replace, or clear rules itself. Remembering a rule from a request requires the owner-authenticated control interface plus that request's still-pending full approval ID; batch replacement (the `--policy` load), rule listing, and clearing require only the owner-authenticated control interface. An allow rule trusts the requested path and exact access, not filesystem identity or independently authenticated provenance. Do not treat this shortcut as a sandbox enforcement layer.
...
Session Rule hits still pass HTTP/body/wire/detail validation and bypass pending capacity checks, producing no approval ID or Tombstone, while reserving the same replay key as a completed request so a repeated webhook cannot collect a second automatic decision. Rules are capped at 128 entries with a 4096-byte path limit; repeated additions replace the same path/scope/access key. Ordinary registration, rule evaluation, remember-and-decide, and clear are serialized under the Broker lock. Rule hits can generate logs and Debug Capture even while pending capacity is full; those diagnostics are not a rate limiter.
