...
Press `S` in the TUI to save the daemon's complete current rule set. The TUI obtains the rules through `GET /v1/session-rules` and writes the same `[[rules]]` format. Bare names resolve under `~/.config/nono-approval/`; relative and absolute paths are used directly. Saving requires at least one rule and creates the destination exclusively, so an existing file causes an error and is never overwritten. Rule origin is not preserved.
...
After HTTP, body, wire, and display validation, a matching rule returns `200 granted` or `200 denied` immediately. Hits create no approval ID, pending entry, Tombstone, or lease and do not consume pending capacity. A hit reserves the same `(session_id, request_id)` replay key as a completed approval for the tombstone TTL, so a repeated webhook receives the duplicate error instead of a second automatic decision, and a key still attached to a pending request is likewise rejected. This shortcut cannot override nono hard denies or authorize operations the daemon never receives. Shutdown disables rule evaluation before denying pending requests.
...
This version has no default policy file, no automatic policy loading, no policy-file environment variable, no restart persistence, and no hot reload; policy loading happens only through the explicit `--policy <file>` load. The TUI `S` key is an export: it writes the daemon's current rules to a new TOML file for a later `--policy` load and never applies a policy itself. The [Rule Scope Selector](rule-editor.md) has no free-form editing, and loading/saving are separate file workflows rather than an in-TUI policy editor. The component matcher supports exact paths, trailing `/` (self plus descendants), `*` (one component), and `**` (one or more components), but runtime clients submit literal exact/subtree rules only. Component-internal `*` patterns are rejected by the glob parser; literal rules do not interpret them as glob source. Middle `**` also consumes at least one component: `/a/**/test` does not match `/a/test`. Restart-persistent rule storage and precedence between persistent and runtime rules remain separate future work.
